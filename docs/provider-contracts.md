...
| U-15 | Meshy：**Free プランでAPIキーを発行できるか**。API利用は Pro 以上の特典として挙げられており、Free の欄には無い。**2026-09-08、利用者の判断で確認を保留**（`docs/decisions.md` D-6）。A3.5 で Meshy を動かす直前に再確認する | 設定の「API」タブでキーを発行できるか | **本システムはAPI経由でしか動かない**。発行できなければ、A3.5の前に Pro（$20/月）の契約が要る |
...
| U-20 | Meshy：**規約第2.9条（Enterprise 以外は学習利用の対象）と第3.2条（有料は非公開に保つ選択ができる）の関係**。有料プランが学習利用から外れるのかどうかが本文だけでは確定できない（第2.7.2節） | Meshy への書面での照会（`legal@meshy.ai`） | **生徒作品を Meshy に送れるかを直接左右する。** 回答が得られるまでは安全側（＝有料でも学習利用の対象）として扱う |
| U-22 | 両社：**「train / validate / test / improve」の具体的な範囲**。どちらの規約にも定義条項が無い（第2.7.1.1節）。とくに Meshy 第2.9条の目的語は `Services` で `as training data` の限定が無く、**AIモデルの学習に限らない**と読める。人手による内容の確認が含まれるか、評価用データとして保管されるかは条文から読み取れない | 両社への書面での照会 | 生徒作品を送る判断に必要。U-20 と同じ照会にまとめて出せる |
