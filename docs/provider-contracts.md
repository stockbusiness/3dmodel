...
#### 1.13.3 1件目の成果物の実測値（**Unity 用途にそのままでは重い**）

A3.5 1件目（壺）の GLB を検査した結果。`tripo-standard`
（`v2.5-20250123` / `texture=True` / `pbr=True` / **`face_limit` 未設定**）。

| 項目 | 実測 | Unity・メタバース用途での評価 |
| --- | --- | --- |
| **三角形数** | **501,190** | **重すぎる。** 置物1つにこの数は現実的でない |
| 頂点数 | 256,683 | **1メッシュで 65,535 を大きく超える**（T-18 の 16bit 境界を超え、32bit インデックスになる） |
| メッシュ数 | 1 | 単一メッシュ |
| マテリアル数 | 1 | — |
| **テクスチャ枚数** | **3** | すべて **2048×2048** |
| **ファイル容量** | **14,645,612 バイト（約 14.0 MiB）** | **重すぎる。** 多数を並べる世界には載らない |
| バウンディングボックス | 約 0.89 × 1.00 × 0.89 | **高さ 1.0 に正規化**されている。実寸ではない |

**この実測が T-17・T-18 の議論に直接効く。**

- **`face_limit` を設定していないため、事業者の既定のまま出ている。**
  プリセットは `face_limit` を渡せる作りになっている（`app/providers/tripo.py`）が、
  **B-7（千ノ国側の上限値）が未定のため設定していない。**
- **`texture_size` の既定は 4096**（第1.11.1節）。今回は 2048 で出ているが、
  `convert_model` を使う場合は明示しないと 4096 になる。
- **高さ 1.0 への正規化**は `auto_size=False`（既定）の挙動と見られる。**未確認。**
  実寸で置くなら `convert_model` の `scale_factor` が要る（U-32）。

**結論：品質の話とは別に、「そのままではメタバースに置けない大きさ」である。**
軽量化の手段は3つあり、いずれも**価格が未確認**である。

| 手段 | 場所 | 未確認事項 |
| --- | --- | --- |
| 生成時の `face_limit` | プリセット（**追加費用なし**の見込み） | 品質への影響 |
| `convert_model(face_limit=...)` | 後処理 | 価格（U-27） |
| `smart_lowpoly` | 後処理 | 価格（U-29） |

**まず生成時の `face_limit` を試すのが筋である**（追加のタスクを増やさないため）。
ただし**B-7（上限値）が決まらないと数値を置けない。**
