...
| U-15 | Meshy：**Free プランでAPIキーを発行できるか**。API利用は Pro 以上の特典として挙げられており、Free の欄には無い。**保留を解除（2026-09-08）**：無料アカウントで A3.5 を実施すると決まった（`docs/decisions.md` T-11）ため、**A3.5 の成否を直接決める関門に戻った** | 設定の「API」タブでキーを発行できるか | **本システムはAPI経由でしか動かない**。発行できなければ Meshy は A3.5 に参加できない |
| U-23 | Tripo：**無料アカウントでクレジットが付与されるか、APIキーを発行できるか**。1credit=$0.01 は確認済みだが、無料枠の有無を確認していない | Tripo の購入画面・API設定画面 | **無料で A3.5 を実施する前提（T-11）の成否を直接決める** |
| U-24 | Meshy：**追加クレジットパックを購入した場合、規約第3.2条の「paid Meshy plan」に該当するか**。該当しなければ、クレジットを買っても生成物は CC BY 4.0（Meshy が権利を保有）のまま | Meshy への照会、または購入後の表示 | 無料枠の不足（3件）を追加クレジットで補う場合に、権利の扱いが変わるかどうかが決まる |
