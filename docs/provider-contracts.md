...
#### 2.4.1 プラン価格（2026-09-08 確認・**JPY建て**）

`https://meshy.ai/ja/settings/subscription` の画面を利用者が提示した。
表示通貨は **JPY**。USD表示は未取得のため、**USD単価は依然として未確認**である。

| プラン | 通常の月額（JPY） | 月次クレジット | 同時タスク | 1クレジットあたり（JPY・計算値） |
| --- | --- | --- | --- | --- |
| Free | ¥0 | 100 | 1 | — |
| Pro | ¥3,084 | 1,000 | 10 | ¥3.084 |
| Premium | ¥6,168 | 3,000 | 30 | ¥2.056 |
| Ultra | ¥15,420 | 8,000 | 100 | ¥1.9275 |

- 画面には「**最初の月**」の割引価格（Pro ¥1,542／Premium ¥3,084／Ultra ¥7,710）が
  大きく出ているが、**恒久単価として使わない**。上の表は「月額請求、¥N/月」と
  併記された通常価格である。
- 1クレジットあたりの金額は**こちらの計算値**であり、事業者が表示した数字ではない。
  実際の請求は月額固定で、クレジットの繰越可否は未確認。
- 税込か税別かは画面から判断できない（未確認）。
- 画面には「個人向け／チーム向け」の切替がある。ここに記録したのは
  Free / Pro / Premium / Ultra の4プランで、公式APIドキュメントのレート制限表が
  Studio / Enterprise を別に挙げていることから、個人向けの表と判断した。
...
| Pro 以上 | 「すべてのアセット用のプライベートライセンス」（画面下端で文が切れており、全文は未取得） |
...
Pro 以上の「プライベートライセンス」の正確な条件は未取得（U-4c は未解消）。
...
| U-3b | Meshy：**クレジットのUSD単価**。クレジット数（30 credits/件）と**JPY建てのプラン価格は確認済み**（第2.4節・第2.4.1節）。USD表示をまだ取得できていない | `https://meshy.ai/settings/subscription` の通貨を USD にした画面 | 上限額を micro-USD で持つ決まりのため、**JPY からの為替換算では確定できない**。**Tripoと同じ単価を仮定しない**（仕様第11章） |
| U-13 | Meshy：追加クレジットパック（単品購入）の価格。プランの月次クレジットを使い切った場合の単価はこちらになる | 購入画面 | 実運用での上限額の根拠が変わる |
