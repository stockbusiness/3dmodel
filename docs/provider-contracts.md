...
**購入後に確かめること：** 本当に $1 = 100credits なのかは、
購入して残高を見るまで確定しない（価格ページの記載からの計算である）。
購入後に `app.cli check-provider tripo --connect` で残高を確認し、
**$2 で 200credits になっているか**を第1.8節に追記する。
違っていた場合は見積単価（`tripo-standard` の $0.30/件）を見直す。
