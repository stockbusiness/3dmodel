...
| U-23 | Tripo：無料アカウントでクレジットが付与されるか、APIキーを発行できるか。**APIキーの発行は確認済み（2026-09-09）：無料アカウントでも「新しい秘密鍵を作成する」が使える**（第1.7節）。**残るのはクレジット残高**で、画面に表示が無いため未確認 | `https://developers.tripo3d.ai/ja/keys`（発行は確認済み）。残高は管理画面の接続テストで分かる | **Tripo は無料でも A3.5 に参加できる見込み**。ただし残高が 0 なら生成できない |
